...
namespace KakaoTalkBot.Utilities
{
    ...
    public class KeyboardHookEventArgs
    {
        ...
        internal KeyboardHookEventArgs(Hook.KBDLLHOOKSTRUCT lParam)
        {
            ...
            this.isRCtrlPressed = Convert.ToBoolean(GetKeyState(VirtualKeyStates.VK_RCONTROL) & KEY_PRESSED) || this.Key == Key.LeftCtrl;
            ...
            if (new[] { Key.LeftAlt, Key.RightAlt, Key.LeftCtrl, Key.LeftCtrl, Key.LeftShift, Key.RightShift, Key.LWin, Key.RWin }.Contains(this.Key))
                this.Key = Key.None;
        }
